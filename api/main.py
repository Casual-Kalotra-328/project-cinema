...
def recommend_by_genres(genres: str, n: int = 3):
    return recommendations(RecommendRequest(genres=[g.strip() for g in genres.split(",")], n=n))
...
def search_movies(q: str, limit: int = 10):
    ...
    results = movies[mask].head(limit)[["movieId","title","genres","release_year"]].to_dict(orient="records")
